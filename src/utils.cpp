// NOTE: The code is copied from [https://github.com/immunogenomics/LISI/blob/master/src/utils.cpp].
 ...
 arma::vec compute_simpson_index(
   ...
   for (int i = 0; i < n ; i++) {
     ...
     while(std::fabs(Hdiff) > tol && tries < 50) {
       if (Hdiff > 0){
         betamin = beta;
         if (!std::isfinite(betamax)) beta *= 2;
         else beta = (beta + betamax) / 2;
       } else{
         betamax = beta;
         if (!std::isfinite(betamin)) beta /= 2;
