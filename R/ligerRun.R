...
#' @param reference Character, numeric or logical selection of one dataset, out of all available datasets in object, to use as a "reference" for quantile normalization.
#' @param minCells Minimum number of cells to consider a cluster shared across datasets
#' @param nNeighbors Number of nearest neighbors for within-dataset knn graph.
#' @param useDims Indices of factors to use for shared nearest factor determination.
#' @param center A logical to center data.
#' @param maxSample Maximum number of cells used for quantile normalization of each cluster and factor
#' @param eps The error bound of the nearest neighbor search.
#' @param refineKNN A logical to increase robustness of cluster assignments using KNN graph.
#' @param clusterName Variable name that will store the clustering result in metadata of a liger object or a Seurat object.
