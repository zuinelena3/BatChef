...
metrics <- function(anndata, cell_type, batch, reduction, only_ARI = FALSE) {
  ...
  out <- basiliskRun(proc = proc, fun = function(anndata, cell_type, batch, reduction) {
    ...
    if(only_ARI == FALSE){
      ...
      casw <- sklearn$metrics$silhouette_score(anndata$obsm[[paste0("X_", tolower(reduction))]], anndata$obs[, cell_type])
      clisi <- median(compute_lisi(anndata$obsm[[paste0("X_", tolower(reduction))]], data.frame(ct = anndata$obs[, cell_type]), "ct")$ct)
