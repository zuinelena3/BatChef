...
seuratV5Run <- function(input, method = "CCAIntegration", orig.reduction = "pca",
                        ...
                        sd.weight = 1, sample.tree = NULL, preserve.order = FALSE,
                        verbose = TRUE, l2.norm = TRUE, k.anchor = 5,
                        k.score = 30, max.features = 200, nn.method = "annoy",
                        n.trees = 50, eps = 0) {

  IntegrateLayers(object = input, method = method, orig.reduction = orig.reduction,
                  assay = assay, features = features, layers = layers,
                  scale.layer = scale.layer,
                  new.reduction = new.reduction, reference = reference,
                  normalization.method = normalization.method,
                  dims = dims, k.filter = k.filter,
                  ...
                  preserve.order = preserve.order, verbose = verbose,
                  l2.norm = l2.norm, k.anchor = k.anchor,
