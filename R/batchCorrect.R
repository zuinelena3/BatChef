...
setMethod("batchCorrect", "SeuratV3Params", function(input, batch, params) {
  ...
  out <- seuratV3Run(input = ll, assay = params@assay,
                     ...
                     anchor.features = params@anchor.features,
                     scale = params@scale,
                     normalization.method = params@normalization.method,
                     sct.clip.range = params@sct.clip.range,
                     reduction = params@reduction,
                     l2.norm = params@l2.norm, dims = params@dims,
                     k.anchor = params@k.anchor, k.filter = params@k.filter,
                     k.score = params@k.score,
                     max.features = params@max.features,
                     nn.method = params@nn.method, n.trees = params@n.trees,
                     eps = params@eps, verbose = params@verbose,
                     new.assay.name = params@new.assay.name,
                     features = params@features,
                     features.to.integrate = params@features.to.integrate,
                     k.weight = params@k.weight,
                     weight.reduction = params@weight.reduction,
                     sd.weight = params@sd.weight,
                     sample.tree = params@sample.tree,
...
setMethod("batchCorrect", "SeuratV5Params", function(input, batch, params) {
  ...
  out <- seuratV5Run(input = so, method = params@method,
                     orig.reduction = params@orig.reduction,
                     assay = params@assay,
                     features = params@features, layers = params@layers,
                     scale.layer = params@scale.layer,
                     new.reduction = params@new.reduction,
                     reference = params@reference,
                     normalization.method = params@normalization.method,
                     dims = params@dims, k.filter = params@k.filter,
                     dims.to.integrate = params@dims.to.integrate,
                     k.weight = params@k.weight,
                     weight.reduction = params@weight.reduction,
                     sd.weight = params@sd.weight,
                     sample.tree = params@sample.tree,
                     preserve.order = params@preserve.order,
                     verbose = params@verbose, l2.norm = params@l2.norm,
                     k.anchor = params@k.anchor, k.score = params@k.score,
                     max.features = params@max.features,
                     nn.method = params@nn.method,
                     n.trees = params@n.trees, eps = params@eps)

  res <- seuratv5Post(input = input, output = out,
                      method = paste0("seuratv5_", tolower(sub("\\I.*", "", params@method))),
                      name = params@new.reduction)
})
...
setMethod("batchCorrect", "SCVIParams", function(input, batch, params) {
  out <- scVIRun(input = input, batch = batch, layer = params@layer,
