...
scVIRun <- function(input, batch, assay_type = "counts", layer = NULL, labels_key = NULL,
  ...
  out <- basiliskRun(
    proc = proc, fun = function(input, batch, assay_type = "counts",
      ...
      arg <- c(list(
        adata = andata, layer = layer, batch_key = batch,
      ...
      arg <- c(list(
        adata = andata, n_hidden = as.integer(n_hidden),
      ...
      arg <- c(list(
        max_epochs = as.integer(max_epochs), accelerator = accelerator,
      ...
      corrected <- model_scvi$get_normalized_expression(
        adata = andata,
        indices = indices,
